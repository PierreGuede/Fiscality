                ...
                <div class=" bg-white shadow-lg z-10 absolute ml-6 w-52 shadow-gray-400" x-show="workToggle">
                    ...
                    <ul class="space-y-2 w-full p-1">
                        @foreach (Auth::user()->company as $company)
                            <li class="w-full"><a href="{{ route('work.show', $company->id) }}"
                                    class="inline-flex items-center w-full px-2 py-1 text-sm font-semibold transition-colors duration-150 rounded-md hover:bg-gray-100 hover:text-gray-800">{{ $company->name }}</a>
                            </li>
                        @endforeach
                    </ul>
                </div>
