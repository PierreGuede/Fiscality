<div
    ...
    <div x-data="{ arrayLimit: @js($arr_sum) , fat_amount: @js($fat_amount) }"
        ...
        <form x-data="globalData" wire:submit.prevent="save">
            ...
            <div class="mt-2 space-y-3 ">
                ...
                <x-input :disabled="true" type="number" label="Total Frais généraux" id="delay_condition" name=""
                         x-bind:value="arrayLimit.reduce((acc, next) => Number(acc) + Number(next) ,0)"
                         class="block w-full" required autofocus/>
                <x-input  type="number" label="Montant des FAT" id="delay_condition" name=""
                         wire:model.defer="fat_amount"
                         x-model="fat_amount"
                         class="block w-full" required autofocus/>

                <x-input :disabled="true" type="number" label="Limite de déduction" id="delay_condition" name=""
                         x-bind:value="arrayLimit.reduce((acc, next) => Number(acc) + Number(next) ,0) * 5/100"
                         class="block w-full" required autofocus/>

                <x-input :disabled="true" type="number" label="Montant à réintéger" id="delay_condition" name=""
                         x-bind:value="$wire.fat_amount - (arrayLimit.reduce((acc, next) => Number(acc) + Number(next) ,0) * 0.5)"
                         class="block w-full" required autofocus/>
            </div>
