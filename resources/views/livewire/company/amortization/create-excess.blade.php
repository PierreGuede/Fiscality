<div class="py-10 px-10 w-full " >
    ...
    <form x-data="{ taux_use: 0, taux_recommended: 0, dotation: 0 }" class="mt-10 space-y-5" wire:submit.prevent="save">
        ...
        <x-input wire:model.defer="data.category_imo" type="text" label="Catégories d'immobilisation" id="category_imo" name="category_imo"
                 value="{{ old('category_imo') }}" class="block w-full" required autofocus />@error('inputs.' . $key . '.account')
            @error('category_imo')
        ...
        <div>
            <x-input x-model="taux_use" wire:model.defer="data.taux_use" type="text" label="Taux d'armortissement utilisé" id="taux_use" name="taux_use"
        ...
        <div>
            <x-input x-model="taux_recommended" wire:model.defer="data.taux_recommended" type="text" label="Taux d'armortissement recommandé" id="taux_recommended" name="taux_recommended"
                     value="{{ old('taux_recommended') }}" class="block w-full" required autofocus />
            @error('taux_recommended')
            <span class="text-xs text-red-600">{{ $message }}</span>
            @enderror
        </div>
        ...
        <div>
            <x-input   type="text" label="Amortisement non déductible" id="ecart" name="ecart"
                       x-bind:value="taux_recommended ? (dotation*(taux_use - taux_recommended)) / taux_recommended : 0 " class="block w-full" required autofocus />
        </div>
