<div x-data="{ arr_sum: @js($arr_sum), rate: {{ $rate  }}  }">
    <form class=" mt-10 grid grid-cols-2 gap-x-12" action=" " wire:submit.prevent="save">


        <div class="space-y-4" x-data="globalData">

            <x-input :disabled="true" wire:model.defer="account_result" type="number" label="Résultat Comptable" id="account_result"
                     name="account_result"
                     value="{{ old('account_result') }}" class="block w-full" required autofocus/>

            <x-input :disabled="true" wire:model.defer="total_reintegration" type="number" label="Total Réintegration"
                     id="total_reintegration" name="total_reintegration"
                     value="{{ old('total_reintegration') }}" class="block w-full" required autofocus/>

            <x-input :disabled="true" wire:model.defer="total_deduction" type="number" label="Total Déduction" id="total_deduction"
                     name="total_deduction"
                     value="{{ old('total_deduction') }}" class="block w-full" required autofocus/>

            <x-input :disabled="true" wire:model.defer="taxable_income_before_restatement_head_office_costs" type="number"
            ...
            <x-input :disabled="true" wire:model.defer="basis_calculating_deduction_limit" type="number"
            ...
            <x-input :disabled="true" wire:model.defer="deductible_head_office_costs" type="number" label="Frais de siège déductibles"
                     id="deductible_head_office_costs" name="deductible_head_office_costs"
{{--                     x-bind:value="deductible_head_office_costs"--}}
                     class="block w-full" required autofocus/>

            <x-input :disabled="true" wire:model.defer="non_deductible_head_office_costs" type="number"
