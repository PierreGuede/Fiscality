    ...
    <div class="p-4 bg-white rounded-lg shadow-xs">
        <div class="flex p-2">
            ...
            <div class="w-1/5 items-center text-center">
                <button type="button" class="bg-green-500 border border-gray-500 text-white font-bold py-2 px-4 rounded-md" @click="showModal = true">Creer un type d'impot</button>
            </div>
        </div>
                                    ...
                                    <div class="bg-white w-11/12 md:max-w-md mx-auto rounded shadow-lg py-4 text-left px-6" x-show="showModalConfirm" @click.away="showModalConfirm = false" >
                                        <div class=" pb-3">
                                            ...
                                            <div class="flex space-x-2 mt-4 text-center mx-auto w-10/12">
                                                ...
                                                <div class="w-1/2">
                                                    <form action="{{ route('typeImpot.delete',$typeImpot->id) }}" method="POST">
                                                        @csrf
                                                        @method('delete')
		...
		<div class="overflow-auto" style="background-color: rgba(0,0,0,0.5)" x-show="showModal" :class="{ 'absolute inset-0 z-10 flex items-center justify-center': showModal }">
			...
			<div class="bg-white w-11/12 md:max-w-md mx-auto rounded shadow-lg py-4 text-left px-6" x-show="showModal" @click.away="showModal = false" >
				...
				<div class="flex justify-between items-center pb-3">
					<p class="text-2xl font-bold">Créer un typeImpote</p>
				...
				<!-- content -->
                <form action="{{ route('typeImpot.store') }}" method="POST" class="space-y-4 p-4">
                    @csrf
