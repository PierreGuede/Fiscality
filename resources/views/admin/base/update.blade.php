<x-admin-space-layout>
    ...
    <div class=" p-4">
        <div class="flex justify-between items-center pb-3">
            <p class="text-2xl font-bold"> {{ __($base->name) }}</p>
        </div>
        <form action="{{ route('base.update', $base->id) }}" method="POST" class="space-y-4 p-4 max-w-lg">
            @csrf
            <div class="space-y-4">
                <x-input class="w-full" for="name"
                type="text" id="name" name="name" label='Nom de la base'
                placeholder="Nom de la base" value="{{ old('name',$base->name) }}" class="" required autofocus />
            </div>
