    ...
    <div class="p-4 bg-white rounded-lg shadow-xs">
        <div class="flex p-2">
            ...
            <div class="w-1/5 items-center text-center">
                <button type="button" class="bg-green-500 border border-gray-500 text-white font-bold py-2 px-4 rounded-md" @click="showModal = true">Creer un centre d'impôt</button>
            </div>
        </div>
                ...
                @foreach ($taxCenter as $taxCenter)
                    <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                        ...
                        <th  scope="row"  class="flex space-x-4 px-6 py-4 font-medium text-gray-900 dark:text-white whitespace-nowrap">
                            <a href="{{ route('taxCenter.edit',$taxCenter->id) }}" class="text-blue-800">Editer</a>
                                    ...
                                    <div class="bg-white w-11/12 md:max-w-md mx-auto rounded shadow-lg py-4 text-left px-6" x-show="showModalConfirm" @click.away="showModalConfirm = false" >
                                        <div class=" pb-3">
                                            ...
                                            <div class="flex space-x-2 mt-4 text-center mx-auto w-10/12">
                                                ...
                                                <div class="w-1/2">
                                                    <form action="{{ route('taxCenter.delete',$taxCenter->id) }}" method="POST">
                                                        @csrf
                                                        @method('delete')
		...
		<div class="overflow-auto" style="background-color: rgba(0,0,0,0.5)" x-show="showModal" :class="{ 'absolute inset-0 z-10 flex items-center justify-center': showModal }">
			...
			<div class="bg-white w-11/12 md:max-w-md mx-auto rounded shadow-lg py-4 text-left px-6" x-show="showModal" @click.away="showModal = false" >
				...
				<div class="flex justify-between items-center pb-3">
					<p class="text-2xl font-bold">Créer un Centre d'impot</p>
				...
				<!-- content -->
                <form action="{{ route('taxCenter.store') }}" method="POST" class="space-y-4 p-4">
                    @csrf
                    <x-label for="rccm" :value="__('Nom')"/>
                    <input type="text" name="name" class="block w-full mt-1 border-gray-300 rounded-md shadow-sm ">
                    <x-label for="rccm" :value="__('Addresse')"/>
                    <input type="text" name="address" class="block w-full mt-1 border-gray-300 rounded-md shadow-sm ">
