<x-admin-space-layout>
    ...
    <div class=" p-4">
        <div class="flex justify-between items-center pb-3">
            <p class="text-2xl font-bold">Créer une provision</p>
        </div>

        <form action="{{ route('accured-charges.store') }}" method="POST" class="space-y-4 p-4 max-w-lg" >
            @csrf
            <div class="space-y-2">
                <x-input class="w-full" for="compte"
                type="number" id="compte" name="compte"  label='Compte'
                placeholder="Nom du compte" class="" required autofocus />

                <x-input class="w-full" for="designation"
                type="text" id="designation" name="designation"  label='Nom de la provision'
                ...
                <div class="mt-4">
                    ...
                    <x-native-select label="type"
                        ...
                        <option value="expense_provisioned">Charges provisionnées</option>
                        <option value="personnal_provision">Provisions sur charges de personnel</option>
                        <option value="provision">provisions pour risque d'exploitation</option>
                    </x-native-select>
                </div>
            </div>
