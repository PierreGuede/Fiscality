            ...
            <table class="w-full" >
                ...
                @foreach($data as $key => $value)
                    <tr class="w-full shadow-lg shadow-blue-200 border-b-4 border-blue-200 bg-white">
                        ...
                        <td class=" text-center text-xs font-semibold py-4  my-10" x-text="formatNumber({{  $value->dotation  }}, false)" ></td>
                        <td class="text-center text-xs font-semibold py-4 px-4  my-10">
                            <div class="flex space-x-4">
                                <a href="{{ route('amortization.depreciation-assets.edit',['company'=>$company->id,'depreciation'=>$value->id]) }}">
                                ...
                                <svg onclick="Livewire.emit('openModal', 'company.amortization.delete-depreciation-assets-handler', {{ json_encode([ $value->id]) }})"  xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-4 h-4 text-red-600 cursor-pointer">
