...
<div x-data="{ openSidebar: true, openDropdown: false }" :class=" openSidebar ? 'w-60' : 'w-20'  "
    ...
    <a :class="!openSidebar && 'justify-center'" class="mt-3 flex w-full items-start px-3" href="#">
        ...
        <div x-show="openSidebar" class="ml-2 text-sm font-bold transition-all" >
            <p>TECIT</p>
            <p class="text-xs font-medium" >{{ auth()->user()->profile->social_reason  }}</p>
        </div>
    </a>
    <div class="w-full px-2 pb-2">
        ...
        <div class="border-t border-blue-300" >
            <x-native-select class="!bg-transparent !border-none focus:!outline-none focus-within:!border-none focus-within:!border-none !shadow-none  " >
                @hasanyrole('cabinet')
                ...
                @if(!empty(auth()->user()->getWorkspaceCompany))
                    @foreach(auth()->user()->getWorkspaceCompany as $workspaceCmpany)
                        <option @selected($company->id == $company->id ) value="{{  $workspaceCmpany->id }}"> {{ $workspaceCmpany->name  }} </option>
                    @endforeach
                @endif
            </x-native-select>
        </div>
    ...
    <div class=" relative mt-auto w-full">
        <button @click=" openDropdown = !openDropdown " type="button" :class="!openSidebar && 'justify-center'"
                class=" flex h-16 w-full items-center justify-start bg-blue-800 px-4 hover:bg-blue-700" href="#">
    ...
    {{--        </x-slot>--}}
    {{--        <x-dropdown.item label="Déconnexion" />--}}
    {{--    </x-dropdown>--}}
