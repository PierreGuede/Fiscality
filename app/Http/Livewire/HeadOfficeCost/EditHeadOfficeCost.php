...
class EditHeadOfficeCost extends Component
{
    public Company $company;

    public HeadOfficeCost $head_office_cost;

    public $head_office_cost_detail;
    ...
    public function mount(Company $company)
    {
        ...
        $this->head_office_cost = HeadOfficeCost::whereCompanyId($this->company->id)->whereYear('created_at', Carbon::now()->year)->first();
        $this->head_office_cost_detail = HeadOfficeCostDetail::whereCompanyId($this->company->id)->whereYear('created_at', Carbon::now()->year)->get();

        $this->fill([
            'inputs' => collect($this->head_office_cost_detail),
            'account_result' => $this->head_office_cost->account_result,
            'total_reintegration' => $this->head_office_cost->total_reintegration,
            'total_deduction' => $this->head_office_cost->total_deduction,
            'taxable_income_before_restatement_head_office_costs' => $this->head_office_cost->taxable_income_before_restatement_head_office_costs,
            'basis_calculating_deduction_limit' => $this->head_office_cost->basis_calculating_deduction_limit,
            'deductible_head_office_costs' => $this->head_office_cost->deductible_head_office_costs,
            'non_deductible_head_office_costs' => $this->head_office_cost->non_deductible_head_office_costs,
        ]);
    }

    public function render()
    {
        return view('livewire.head-office-cost.edit-head-office-cost');
    }
    ...
    public function save()
    {
        ...
        $this->head_office_cost->save();

        for ($i = 0; $i < count($this->inputs); $i++) {
            HeadOfficeCostDetail::updateOrCreate([
                'account' => $this->inputs[$i]['account'],
                'name' => $this->inputs[$i]['name'],
                'amount' => $this->inputs[$i]['amount'],
                'company_id' => $this->company->id,
                'head_office_cost_id' => $this->head_office_cost->id,
            ], ['head_office_cost_id']);
        }
