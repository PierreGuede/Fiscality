...
class EditMinimumTax extends ModalComponent
{
    ...
    public function mount(Company $company)
    {
        ...
        $this->minimumDetail = MinimumTaxDetail::whereCompanyId($this->company->id)->whereYear('created_at', Carbon::now()->year)->whereTypeImpotId($typeImpot->id)->get();
    ...
    {
        return view('livewire.corporate-tax.edit-minimum-tax');
    }
...
//        dd($this->inputs);
        try {
            ...
            $typeImpot = TypeImpot::whereCode(TypeImpot::IS)->first();
            $minimumtax = MinimumTax::whereCompanyId($this->company->id)->whereYear('created_at', Carbon::now()->year)->whereTypeImpotId($typeImpot->id)->first();
